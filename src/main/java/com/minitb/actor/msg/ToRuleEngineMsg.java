 ...
 * 
 * 职责:
 * - 携带业务消息 Message (包含业务消息类型: POST_TELEMETRY_REQUEST, ALARM, etc.)
 * - RuleEngineActor 会根据 Message.type 决定如何处理业务逻辑
 * 
 * 示例:
 * ActorMsgType = TO_RULE_ENGINE_MSG (Actor层: 告诉系统发给 RuleEngineActor)
 * Message.type = POST_TELEMETRY_REQUEST (业务层: 告诉规则引擎这是遥测数据)
 */
@Data
@AllArgsConstructor
public class ToRuleEngineMsg implements MiniTbActorMsg {
    
    private final Message message;  // 包含业务消息类型和数据
